...
onProcessStepChanged = function () {
	...
	switch (startFromStep) {
		case greyscaleStepNum:
			produceGreyscale();
			break;
		case blurStepNum:
			produceImageBlurred();
			break;
		case edgeDetectStepNum:
			produceImageEdges();
			break;
		case thresholdStepNum:
			produceImageThresholds();
			break;
		case dilateStepNum:
			produceImageDilate();
...
onFileInput = function () {
	...
	console.log("File: " + fileElement.files[0]);
...
produceImageDilate = function () {
	// dilated
	createDilateKernel();
	dilated = dilateKernel.convoluteMax(thresholded, {
		earlyOutFn: accum => accum >= 1
	});
